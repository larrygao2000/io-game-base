  ...
  emit(event, context) {
    ...
    if (this.player.lockPlayer) {
      if (Math.random() < 0.1) {
        ...
        } else if (this.player.distanceTo(this.player.lockPlayer) > Constants.PLAYER_RADIUS * 3) this.player.setMoveDirection(this.player.lockPlayer.speed, this.player.lockPlayer.direction);
      }
      return;
    }
